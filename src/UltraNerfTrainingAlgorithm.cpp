...
namespace ImFusion
{
	UltraNerfTrainingAlgorithm::UltraNerfTrainingAlgorithm(SharedImageSet *imgIn)
		: m_imgIn(imgIn)
	{
	}

	UltraNerfTrainingAlgorithm::~UltraNerfTrainingAlgorithm()
	{
	}

	bool UltraNerfTrainingAlgorithm::createCompatible(const DataList &data, Algorithm **a)
	{
		...
		if (a)
		{
			*a = new UltraNerfTrainingAlgorithm(img);
		}
		return true;
	}

	// This function does all of the work of this class
	void UltraNerfTrainingAlgorithm::compute()
	{
	...
	OwningDataList UltraNerfTrainingAlgorithm::takeOutput()
	{
		// if we have produced some output, add it to the list
		return OwningDataList(std::move(m_imgOut));
	}

	void UltraNerfTrainingAlgorithm::configure(const Properties *p)
	{
	...
	void UltraNerfTrainingAlgorithm::configuration(Properties *p) const
	{
